...
process.env.SCRIPT ? runAsScript() : runAsServer();
...
async function runAsServer() {
  ...
  apiRouter.get("/validate", authenticationController.handleValidationRequest);
  ...
  const socketSubcontrollers = [
    new ChatSubcontroller(),
    new BlackjackSubcontroller(),
  ];

  for (const subcontroller of socketSubcontrollers) {
...
async function runAsScript() {
  ...
  const scripts = (await import("./scripts")) as Record<string, () => unknown>;
